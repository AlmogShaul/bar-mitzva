...
interface GroupedVerseCardProps {
  psukim?: Pasuk[];
}
...
export const GroupedVerseCard: React.FC<GroupedVerseCardProps> = ({ psukim }) => {
  ...
  const handlePlay = async () => {
    if (!audioRef.current || !psukim || psukim.length === 0) return;
    ...
    try {
      ...
      if (isPlaying) {
        ...
        const groupIds = psukim.map(pasuk => `${pasuk.chapter}_${pasuk.pasuk}`).join(',');
        audioRef.current.src = `/api/audio/group?ids=${groupIds}`;
  ...
  const compareWithRabbi = async () => {
    if (!sessionId || !psukim || psukim.length === 0) {
    ...
    try {
      ...
      const pasukIds = psukim.map(pasuk => pasuk.id); // Collect IDs of all psukim in the group
      ...
      const response = await fetch(`${API_BASE}/compare-audio`, {
        ...
        body: JSON.stringify({ session_id: sessionId, pasuk_ids: pasukIds }),
      });
  ...
  return (
    <div className="grouped-verse-card">
      {psukim.map((pasuk, index) => (
